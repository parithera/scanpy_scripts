...
class LinkToProject:
    ...
    def run(self):
        ...
        organization_folder = self.output_path.split("projects")[0]
        
        with open(os.path.join(self.output_path, "groups.json")) as f:
            samples = json.load(f)
        ...
        adatas_combined.write_h5ad(
            self.output_path.replace("python", "out.h5ad"),
            compression=hdf5plugin.FILTERS["zstd"]
        )

if __name__=='__main__':
    output_path = sys.argv[1]
    link_to_project = LinkToProject(output_path)
    link_to_project.run()